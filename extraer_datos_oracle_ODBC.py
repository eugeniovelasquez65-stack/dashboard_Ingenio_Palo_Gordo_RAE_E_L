...
def conectar_oracle():
    try:
        # Usar el DSN de Toad (Produccion)
        conn_str = (
            f"Driver={{Oracle in instantclient_19_29}};"
            f"Dbq={ORACLE_HOST}:{ORACLE_PORT}/{ORACLE_SID};"
            f"Uid={ORACLE_USER};Pwd={ORACLE_PASSWORD};"
        )
        
        connection = pyodbc.connect(conn_str)
        print("Conexion ODBC exitosa")
        return connection
        ...
        try:
            ...
            connection = pyodbc.connect(conn_str_tns)
            print("Conexion ODBC (TNS) exitosa")
            return connection
        except Exception as e2:
            print(f"Error TNS: {e2}")
            return None
...
def extraer_todos_los_datos():
    print("="*70)
    print("EXTRACCION DE DATOS (ODBC)")
    print("="*70)
